...
explore_asn <- mp %>%
  ...
  select(CREW_ID, BID_PERIOD, PAIRING_NO, PAIRING_DATE, PAIRING_POSITION) %>% 
  ungroup()
...
arcs <- inner_join(explore_arcs, clean_arc) %>% 
  filter(CREW_ID %in% c(rlv_FAs$CREW_ID)) %>% 
  group_by(CREW_ID, PAIRING_DATE, PAIRING_NO) %>% 
  mutate(temp_id = cur_group_id()) %>% 
  filter(!duplicated(temp_id)) %>% 
  select(!temp_id)


# Need to capture most recent Pairing number per date but also any pairing numbers for ARC
cred_final <- inner_join(explore_asn, mid_pm) 


final <- rbind(cred_final, arcs)


#final <- inner_join(cred_final, cred_ms, by = c("PAIRING_NO", "PAIRING_DATE", "FLIGHT_NO"))
```
