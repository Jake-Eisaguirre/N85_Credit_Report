...
```{r}
final_tran <- ms %>%
  filter(TRANSACTION_CODE %in%  c("VC1", "VC2", "MIL", "UNI", "2CB", "2SK", "FLV", "FLP")) %>% 
  filter(CREW_ID %in% rlv_FAs$CREW_ID) %>%
  group_by(CREW_ID, PAIRING_DATE, TO_DATE) %>% 
  mutate(update_dt = paste(UPDATE_DATE, UPDATE_TIME, sep = " "),
         #fill_complete_id = cur_group_id()
         ) %>%
  ungroup() %>% 
  ...
  filter(combined_credit == min(combined_credit)) %>% 
  ungroup() %>% 
  group_by(CREW_ID, TRANSACTION_CODE) %>% 
  mutate(flag_2 = if_else(!PAIRING_DATE == TO_DATE, 1, 0)) %>% 
  filter(flag_2 == 0) %>% 
  select(!c(flag, flag_2))
# %>%
#   pivot_longer(cols = c(PAIRING_DATE, TO_DATE), names_to = "date_type", values_to = "PAIRING_DATE") %>%
#   group_by(CREW_ID, TRANSACTION_CODE, fill_complete_id) %>%
#   complete(PAIRING_DATE = seq(min(PAIRING_DATE), max(PAIRING_DATE), by = "day")) %>%
#   fill(CREW_ID, BID_PERIOD, BASE, PAIRING_NO, TRANSACTION_CODE, PAIRING_POSITION, combined_credit, .direction = "downup") %>%
#   ungroup() %>%
#   select(!c(date_type, fill_complete_id)) %>%
#   group_by(CREW_ID, TRANSACTION_CODE, PAIRING_DATE) %>%
...
pm_q <- paste0("select PAIRING_DATE, PAIRING_NO, FLIGHT_NO, CREW_INDICATOR, PAIRING_POSITION, BID_TYPE, 
         ACT_CREDIT_CO_DOM_TIME, ACT_CREDIT_CO_INT_TIME, ACT_CREDIT_DOM_TIME, 
         ACT_CREDIT_INT_TIME, ACT_DEADHEAD_CREDIT_CO_DOM_TIME, ACT_DEADHEAD_CREDIT_CO_INT_TIME, ACT_DH_CREDIT_DOM_TIME,
         ACT_DH_CREDIT_INT_TIME, UPDATE_DATE, UPDATE_TIME
