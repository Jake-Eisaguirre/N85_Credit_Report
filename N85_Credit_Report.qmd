...
final_tran <- mh %>%
  ...
  select(CREW_ID, BID_PERIOD, BASE, PAIRING_NO, TRANSACTION_CODE, PAIRING_DATE, TO_DATE, PAIRING_POSITION, CREDIT, update_dt) %>% 
...
cred_final <- inner_join(explore_asn, mid_pm) %>% 
  ...
  ungroup() %>% 
  filter(!PAIRING_ASSIGNMENT_CODE %in% c("O75", "TTD", "TSD", "SOP", "VNO", "FLU", "PLS", "UNA", "NOS", "XXX", "FLP", "FLV"),
